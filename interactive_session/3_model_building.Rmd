...
First, let's load in some packages: `here`,`broom`, `tidyr`, `dplyr`, `janitor`, `visdat`, and `caret`.
...
Ok, now we're going to build a simple predictive model with our covariates. We're going to use the `select` function in the `dplyr` package to pick our variables from the larger dataset.
...
We're going to use `visdat` from the `visdat` library to summarize our data again. Does everything look good?
```{r}
...
- `partition` creates two (or more, if you'd like) partitions from a single dataset
...
- You can set the size of each partition by setting `p` (percent) equal to a number between 0 and 1.
  ...
  You do not need to set a size for the last partition, since it is inferred.
...
```{r}
set.seed(123) ## setting a seed produces the same results each time. this is great for reproducibility!
...
- Patient 2 is 50 years younger than Patient 1, but has the same probability of readmission because they stayed in the hospital twice as long.
- Patient 3 is only about 15 years older than Patient 2 (and 30+ years younger than Patient 1), but has nearly the same risk because their hospital stays were similar.
- Patient 4, well, we'll probably see Patient 4 again soon.


It sort of seems like `age` doesn't matter, but we can test this using `tidy` on `basic_model`.
Predictor variables aren't all equally important in a model.
They're actually weighted in terms of importance, and we can see that by looking at the `estimate`.
...
predictions2 <- predictions2 %>% 
  mutate(readmit30 = factor(readmit30),
         predict_readmit30 = factor(predict_readmit30))
...
hosp_readmit_data_filtered2 <- hosp_readmit_data %>% 
    select(patient_id, readmit30, length_of_stay## fill in with your variables!
           ) %>%
...
partitioned_data2 <- partition(hosp_readmit_data_filtered2,
                              p = 0.85,
                              id_col = 'patient_id',
                              cat_col = 'readmit30')
